...
public class BinanceFutureTimestampFactory implements SynchronizedValueFactory<Long> {
	...
	public BinanceFutureTimestampFactory(BinanceFuture binance,
	...
	private BinanceTime binanceTime() throws IOException {
		return ResilienceUtils.decorateApiCall(resilienceSpecification,
				() -> binance.time()).withRetry(resilienceRegistries.retries().retry("time"))
				.withRateLimiter(resilienceRegistries.rateLimiters().rateLimiter(REQUEST_WEIGHT_RATE_LIMITER)).call();
	}
}
